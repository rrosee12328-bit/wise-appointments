...
export const syncAcuityAppointments = createServerFn({ method: "POST" }).handler(async () => {
  ...
  const apptRes = await fetch(`${ACUITY_API_BASE}/appointments?${params}`, {
    headers: {
  ...
  for (const appt of appointments) {
    ...
    const clientName =
      [appt.firstName, appt.lastName].filter(Boolean).join(" ") || appt.email || "Unknown Client";
  ...
  try {
    await syncGoogleBlocksForUser(userId, "acuity");
  } catch (e) {
    console.error("acuity: syncGoogleBlocksForUser failed", e);
  }
  try {
    await syncOutlookBlocksForUser(userId, "acuity");
  } catch (e) {
    console.error("acuity: syncOutlookBlocksForUser failed", e);
  }
