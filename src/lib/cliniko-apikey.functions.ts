...
export const syncClinikoAppointments = createServerFn({ method: "POST" }).handler(async () => {
  const authHeader = getRequestHeader("authorization");
  const token = authHeader?.replace(/^Bearer\s+/i, "");
  if (!token) throw new Error("Not authenticated");
  ...
  try {
    await syncGoogleBlocksForUser(userId, "cliniko");
  } catch (e) {
    console.error("cliniko: syncGoogleBlocksForUser failed", e);
  }
