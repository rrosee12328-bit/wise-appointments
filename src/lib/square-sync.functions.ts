...
export const syncSquareBookings = createServerFn({ method: "POST" }).handler(
  async () => {
    ...
    if (!accessToken || expiresAt - Date.now() < 60_000) {
      if (!refreshToken)
        throw new Error("No refresh token. Please reconnect Square.");
    ...
    for (const booking of bookings) {
      ...
      if (booking.status === "CANCELLED_BY_SELLER" || booking.status === "CANCELLED_BY_CUSTOMER" || booking.status === "NO_SHOW") {
        skipped++;
        continue;
      }
      ...
      const clientName = customer
        ? [customer.given_name, customer.family_name]
            .filter(Boolean)
            .join(" ") || customer.email_address || "Unknown Client"
