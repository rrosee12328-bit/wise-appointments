...
export const syncZohoBookings = createServerFn({ method: "POST" }).handler(
  async () => {
    ...
    if (!accessToken || expiresAt - Date.now() < 60_000) {
      if (!refreshToken)
        throw new Error("No refresh token. Please reconnect Zoho Bookings.");
    ...
    for (const appt of appointments) {
      ...
      const externalId =
        appt.booking_id ?? `${appt.customer_email}-${startsAt}`;
      const clientName =
        appt.customer_name || appt.customer_email || "Unknown Client";
