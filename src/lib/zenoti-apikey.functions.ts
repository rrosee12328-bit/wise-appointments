...
export const connectZenotiApiKey = createServerFn({ method: "POST" })
  .inputValidator((input) => z.object({ apiKey: z.string().min(10) }).parse(input))
  .handler(async ({ data }) => {
...
export const syncZenotiAppointments = createServerFn({ method: "POST" }).handler(async () => {
  const authHeader = getRequestHeader("authorization");
  const token = authHeader?.replace(/^Bearer\s+/i, "");
  if (!token) throw new Error("Not authenticated");
  ...
  for (const appt of appointments) {
    ...
    const clientName =
      [appt.guest?.first_name, appt.guest?.last_name].filter(Boolean).join(" ") ||
      appt.guest?.email ||
      "Unknown Client";
  ...
  try {
    await syncGoogleBlocksForUser(userId, "zenoti");
  } catch (e) {
    console.error("zenoti: syncGoogleBlocksForUser failed", e);
  }
