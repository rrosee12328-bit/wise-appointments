...
export const syncOutlookCalendar = createServerFn({ method: "POST" }).handler(async () => {
  ...
  const calendarIds: string[] =
    outlookCalendars.length > 0 ? outlookCalendars.map((c) => c.id) : ["default"];
  ...
  for (const ev of items) {
    ...
    const bodyText =
      ev.body?.content && ev.body.contentType === "text" ? ev.body.content : (ev.bodyPreview ?? "");
  ...
  try {
    await syncGoogleBlocksForUser(userId, "outlook_calendar");
  } catch (e) {
    console.error("outlook: syncGoogleBlocksForUser failed", e);
  }
  try {
    await cleanupCalendarDuplicates(userId, "outlook_calendar");
  } catch (e) {
    console.error("outlook: cleanupCalendarDuplicates failed", e);
  }
  try {
    await dedupeCrossCalendarRows(userId, "outlook_calendar");
  } catch (e) {
    console.error("outlook: dedupeCrossCalendarRows failed", e);
  }
  try {
    await retagRelayEvents(userId);
  } catch (e) {
    console.error("outlook: retagRelayEvents failed", e);
  }
